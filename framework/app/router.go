...
// 注册路由
func registerRouter(engine *gin.Engine, routerGroup RouterGroup) {
	...
	for _, routerPath := range routerGroup.Routers {
		...
		case http.MethodGet:
			group.GET(routerPath.RelativePath, routerFunc(routerPath.RouterFunc))
		case http.MethodPost:
			group.POST(routerPath.RelativePath, routerFunc(routerPath.RouterFunc))
		case http.MethodPut:
			group.PUT(routerPath.RelativePath, routerFunc(routerPath.RouterFunc))
